...
#define ESCOLHER_VERSAO(v1, v2, v3)\
     if (qrcode.tamanhoDa_mensagemAserCriptografada <= v1) {\
            ...
            qrcode.tabela.bitsdeRemanescenteRequeridos=0;\
     }else if (qrcode.tamanhoDa_mensagemAserCriptografada <= v2) {\
            qrcode.tabela.version = 2;\
            qrcode.tabela.bitsdeRemanescenteRequeridos=7;\
...
void CODF_ETAPA4() {
    ...
    //converter o numero de tamanhoDa_mensagemAserCriptografada para binario
    int i, j;
    qrcode.qtBitsMode = (qrcode.MODE_TYPE == MODO_NUMERICO) * 10 + (qrcode.MODE_TYPE == MODO_ALPHANUMERICO) * 9 + (qrcode.MODE_TYPE == MODO_BYTE) * 8;
    converterParaBinario(
            qrcode.indicadorDecontagemDeCaracteres, qrcode.tamanhoDa_mensagemAserCriptografada, qrcode.qtBitsMode);
    // unir a seq de 4 bits da etapa anterior com a sequencia de bits atual(numero de letras em binario)
...
void modoAlphanumerico() {
    ...
    for (i = 0; i < qrcode.tamanhoDa_mensagemAserCriptografada / coluna; i++) {
        valor = alphaValue(
                qrcode.mensagemAserCriptografada[i * coluna], qrcode.mensagemAserCriptografada[i * coluna + 1]);
...
void modoByte() {
    ...
    for (i = 0; qrcode.mensagemAserCriptografada[i]; i++) {
        ...
        fprintf(
                logFile, "   [%c  %d ] %s\n", qrcode.mensagemAserCriptografada[i], qrcode.mensagemAserCriptografada[i], strfim + i * 8);
    }
...
void modoNumerico() {
    ...
    for (i = 0; i < qrcode.tamanhoDa_mensagemAserCriptografada; i += coluna) {
        integerValueOf(
                qrcode.mensagemAserCriptografada + i, qrcode.mensagemAserCriptografada + (i / coluna) * coluna + coluna, qrcode.mensagemAserCriptografada + qrcode.tamanhoDa_mensagemAserCriptografada, &a);
        fprintf(logFile, "   %d     ", a);
        if (a < 10) {
